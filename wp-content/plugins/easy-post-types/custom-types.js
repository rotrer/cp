...
contexflow.triggers.saveCategoriesUsedByContentType = function(params, el){
  ...
  var pp_to_update = {};
  $('#category-list :checkbox[name=postpages]').each(function(){
    var $this = $(this);
    if($this.attr('checked') != ($this.attr('data-original-state') == 'checked')){
      pp_to_update[$this.val()] = $this.attr('checked');
    }
  });
  
  var refine_to_update = {};
  $('#category-list :checkbox[name=refinesearch]').each(function(){
    var $this = $(this);
    if($this.attr('checked') != ($this.attr('data-original-state') == 'checked')){
      refine_to_update[$this.val()] = $this.attr('checked');
    }
  });


  if(!$.isEmptyObject(cats_to_update) || !$.isEmptyObject(pp_to_update)){
    // make AJAX call to update
    var data = {
      action: 'save_categories_used_by_content_type',
      content_type: params.name,
      categories_to_update: cats_to_update,
      postpages_to_update: pp_to_update,
	  refine_to_update: refine_to_update
    };
