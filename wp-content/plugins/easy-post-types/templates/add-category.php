...
?>
  ...
  <tr>
    ...
    <td>
      <p>
          <?php if (!$_POST['new_category']) : $on = $features_state['hierarchical']?'on':'off'; ?>
          <input type="hidden" name="hierarchical" value="<?php echo $on; ?>" />
          <?php endif; ?>
      ...
      <p>
        <input type="checkbox" id="add-postpages" <?php echo $category['postpages']? 'checked="checked"' : '';?> name="postpages" />
        <label for="add-postapges"><?php _e('Post & Pages','cct'); ?></label>
      </p>
