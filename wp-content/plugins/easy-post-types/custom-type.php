   ...
   public function selectContentType() {
    ...
    public function ajax_save_categories_used_by_content_type() {
      ...
      if(empty($_POST['content_type'])){
        ...
        if (is_array($_POST['categories_to_update'])) {
            foreach($_POST['categories_to_update'] as $key => $state){
              if($state == 'true' || $state=='checked'){
                // Add category to post type
                if (!in_array($_POST['content_type'], $this->fields_info['categories'][$key]['object_type'])) {
            ...
            foreach($this->fields_info['categories'] as $key=>$type) {
                $i=0;
                foreach($this->fields_info['categories'][$key]['object_type'] as $key1=>$value) {
                    if ($value==$_POST['content_type']) {
                        unset($this->fields_info['categories'][$key]['object_type'][$i]);
                    }
                    $i++;
                }
            }
    ...
    public function ajax_export_content() {
        $type=$_POST['content_type'];
        if ($type=='all'){
            foreach($this->fields_info['types'] as $type=>$content) {
                unset($this->fields_info['types'][$type]['register_meta_box_cb']);
            }
    ...
    public function remove_objects(&$var){
        ...
        foreach($var as $key => $t) {
                if (isset($var[$key]['register_meta_box_cb'])) {
                    unset($var[$key]['register_meta_box_cb']);
                }
		}
    }

    public function save_content_type() {
        $this->remove_objects($this->fields_info['types']);
        update_option('ct_content_types', $this->fields_info['types']);
        $this->remove_objects($this->fields_info['fields']);
        update_option('ct_fields_types', $this->fields_info['fields']);
        $this->remove_objects($this->fields_info['categories']);
        update_option('ct_categories_types', $this->fields_info['categories']);
    }

    public function update_version() {
        switch ($this->version) {
            // Version 1.0.1 adds postpages variables to categories.
            // It allows to add custom category to posts and pages. Default = false
            case "1.0.1":
                $ct = get_option('ct_categories_types');
                if ($ct) {
                    $this->fields_info['categories']=$ct;
                    foreach($this->fields_info['categories'] as $key=>$cat) {
                        $this->fields_info['categories'][$key]['postpages']=false;
                    }
                    update_option('ct_categories_types', $this->fields_info['categories']);
                }
    ...
    public function load_content_type() {
        $ct = get_option('ct_content_types_version');
        if ($ct && $ct!=$this->version) {
            $this->update_version();
        }

        update_option('ct_content_types_version', $this->version);
    ...
    public function registerContentTypes() {
        ...
        if (isset($this->fields_info['categories'])) {
            foreach($this->fields_info['categories'] as $key=>$category) {
                if (!empty($category['postpages']))
                    $postpages=array('post','page');
                else
                    $postpages=array();
                register_taxonomy($category['internal_name'], array_merge($category['object_type'],$postpages), $category['filters']);
            }
        }
    }
    ...
    public function custom_fields_meta_box_for_post_edit($post) {
        ...
        foreach($this->fields_info['fields'][$post->post_type] as $field) {
          ...
          $fieldType=isset($field['type']) ? $this->getFieldType($field['type']) : null;
          if ($fieldType!=null) {
...
function get_easy_post_type() {
    global $ept_cf;
    return $ept_cf;
}

function get_easy_post_type_name($slug) {
    global $ept_cf;
    $name = $ept_cf->fields_info['types'][$slug]['singular_label'];
    return $name;
}
...
function get_ept_field($name, $options=null) {
    ...
    if (isset($options['raw']) && $options['raw']==true )
        return get_easy_post_type()->theme($post, $name, $options);
    ob_start();
    get_easy_post_type()->theme($post, $name, $options);
    return ob_get_clean();
}

function get_ept_field_from_post($name, $post, $options=null) {
    if (isset($options['raw']) && $options['raw']==true )
        return get_easy_post_type()->theme($post, $name, $options);
    ob_start();
    get_easy_post_type()->theme($post, $name, $options);
