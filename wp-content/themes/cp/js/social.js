;(function ( $, window, document, undefined ) {
	...
	popup = {
		...
		ok: function(opt){
			PopupCenter('http://www.odnoklassniki.ru/dk?st.cmd=addShare&st.s=1&st._surl='+encodeURIComponent((opt.buttons.ok.url !== '' ? opt.buttons.ok.url : opt.url))+'&title='+encodeURIComponent(opt.text), 'ok', us_script.ok_width, us_script.ok_height );
		},
		weibo: function(opt){
		...
		whatsapp: function(opt){
			PopupCenter('whatsapp://send?text='+encodeURIComponent((opt.buttons.whatsapp.url !== '' ? opt.buttons.whatsapp.url : opt.url))+' '+encodeURIComponent(opt.text), 'whatsapp', us_script.whatsapp_width, us_script.whatsapp_height );
		},
		meneame: function(opt){
			PopupCenter('http://www.meneame.net/submit.php?url='+encodeURIComponent((opt.buttons.meneame.url !== '' ? opt.buttons.meneame.url : opt.url))+'&title='+encodeURIComponent(opt.text), 'meneame', us_script.meneame_width, us_script.meneame_height );
		},
		digg: function(opt){
			PopupCenter('http://digg.com/submit?url='+encodeURIComponent((opt.buttons.digg.url !== '' ? opt.buttons.digg.url : opt.url))+'&title='+encodeURIComponent(opt.text), 'digg', us_script.digg_width, us_script.digg_height );
		}
	};
	...
	Plugin.prototype.init = function () {
		...
		if(this.options.urlCurl !== ''){
			...
			urlJson.pocket = this.options.urlCurl + '?url={url}&type=pocket&action=us_counts';
			urlJson.love = this.options.urlCurl + '?url={url}&type=love&action=us_counts';
			urlJson.ok = this.options.urlCurl + '?url={url}&type=ok&action=us_counts';
			urlJson.xing = this.options.urlCurl + '?url={url}&type=xing&action=us_counts';
			urlJson.managewp = this.options.urlCurl + '?url={url}&type=managewp&action=us_counts';
...
(function($) {
  var defaults = {
      ...
      bottomSpacing: 0,
      className: 'us_sticky',
      wrapperClassName: 'us_sticky_wrapper',
      center: false,
      getWidthFrom: '',
...
jQuery(document).ready(function() {
		...
		jQuery('.us_pocket').each(function() {
			var selector = jQuery(this);
			var share = true;
			var template = '<a class="us_box" href="#"><div class="us_share"><i class="us-icon-pocket"></i></div><div class="us_count">{total}</div></a>';
			if (jQuery(this).hasClass('us_transient') || jQuery(this).hasClass('us_no_count')) {
				var share = false;
				var template = '';
				var count = jQuery(this).data('count');
		...
		jQuery('.us_ok').each(function() {
			var selector = jQuery(this);
			var share = true;
			var template = '<a class="us_box" href="#"><div class="us_share"><i class="us-icon-ok"></i></div><div class="us_count">{total}</div></a>';
			if (jQuery(this).hasClass('us_transient') || jQuery(this).hasClass('us_no_count')) {
				var share = false;
				var template = '';
				var count = jQuery(this).data('count');
		...
		jQuery('.us_xing').each(function() {
			var selector = jQuery(this);
			var share = true;
			var template = '<a class="us_box" href="#"><div class="us_share"><i class="us-icon-xing"></i></div><div class="us_count">{total}</div></a>';
			if (jQuery(this).hasClass('us_transient') || jQuery(this).hasClass('us_no_count')) {
				var share = false;
				var template = '';
				var count = jQuery(this).data('count');
		...
		jQuery('.us_managewp').each(function() {
			var selector = jQuery(this);
			var share = true;
			var template = '<a class="us_box" href="#"><div class="us_share"><i class="us-icon-managewp"></i></div><div class="us_count">{total}</div></a>';
			if (jQuery(this).hasClass('us_transient') || jQuery(this).hasClass('us_no_count')) {
				var share = false;
				var template = '';
				var count = jQuery(this).data('count');
		...
		jQuery('.us_meneame').each(function() {
			jQuery(this).ultimate_social_deux({
				share: {
					meneame: true
				},
				click: function(api, options){
					api.openPopup('meneame');
					return false;
				}
			});
		});
		jQuery('.us_digg').each(function() {
			jQuery(this).ultimate_social_deux({
				share: {
					digg: true
				},
		...
		jQuery('.us_whatsapp').each(function() {
			jQuery(this).ultimate_social_deux({
				share: {
					whatsapp: true
				},
		...
		jQuery(".us_fan_count").each(function(){
			var network = jQuery(this).data('network');
			var us_ajax = jQuery(this).data('ajax');
			var url = us_script.ajaxurl + '?action=us_fan_counts&network=' + network;

			if (us_ajax) {
	          	jQuery.getJSON( url, function(data) {
	          		jQuery('.us_'+data.network+'_fan_count').each(function(){
	          			jQuery(this).find('.us_fan_count_holder').html(data.count);
	          		});
	        	});
        	}
      	});
      	jQuery(".us_more").each(function(){
			jQuery(this).find('.us_box').toggle(function() {
				jQuery(this).parent().parent().find('.us_after_more').removeClass("us_display_none");
				jQuery(this).parent().find('.us_share').html("<i class='us-icon-minus'></i>");
				return false;
			}, function() {
				jQuery(this).parent().parent().find('.us_after_more').addClass("us_display_none");
				jQuery(this).parent().find('.us_share').html("<i class='us-icon-plus'></i>");
				return false;
			});
      	});
	});
